...
- Composite/overlays: `composeRGB`, `renderImages`, overlay preview/save/load callbacks, and the current in-memory implementations in `internal/ui/workspace_compose.go` and `internal/processing`.
...
2. [ ] Restore every channel-local stretch/control operation and bounded previews.
   ...
   - Introduce one large-mode channel-job coordinator used by standard channels and overlays. It snapshots the artifact generation and metadata, runs off the UI thread, ignores stale completion, and commits metadata/artifact/preview together.
...
3. [ ] Make project save/load and reset artifact-aware.
   ...
   - Keep save output path-based and backward compatible; persist channel/overlay state, offsets, quarter-turn rotation, fitted affine, and RGB levels exactly as normal mode does, but never persist session artifact paths. Save must work in either mode.
...
5. [ ] Add transactional artifact transforms for normalize-scale and quarter-turn rotation.
   ...
   - Tests compare small raster output with `ResizeChannel` and `rotateComposeChannel90CW`, cover rectangular rotations and four-turn identity, injected failures/cancellation, dimension metadata, and bounded tile use.
...
6. [ ] Build the sequential disk compositor and restore RGB/overlay preview.
   ...
   - Define an artifact-based immutable render request for B/G/R descriptors and metadata, ordered overlay descriptors/settings, RGB levels, context, and destination artifacts. Keep UI/session ownership out of `processing`.
   ...
   - Blend overlays in their current order/settings by streaming each overlay into the on-disk R/G/B outputs. Preserve validity masks, opacity, highlight protection, tint, and render fingerprint without hashing full pixels in memory.
   - Generate the composite viewport from a downsampled disk composite and streaming histograms. Wire `composeRGB`, `refresh`, measurement coordinates, and RGB Levels to this result; remove “Composite disabled in disk-backed mode.”
...
7. [ ] Reimplement Cross-Channel Clean as a bounded tiled job.
...
8. [ ] Add bounded grayscale and composite export for 8-bit and 16-bit output.
   ...
   - Add encoder-facing disk-backed image/row sources so `saveChannelGray`, `saveLayerGray`, and `exportRGB` consume artifacts without full RGBA or float-channel allocations. PNG 16-bit must quantize rows from disk; PNG/JPEG/TIFF/WebP 8-bit must use a bounded adapter or a format-specific streaming path. Preserve options, extensions, RGB Levels, overlay blend, and atomic destination behavior.
...
9. [ ] Preserve the Send Composite to Edit operation without crossing an in-memory boundary.
...
10. [ ] Remove temporary large-mode guards and run integration/regression validation.
...
- All channel controls, copy/match/normalize/align/clean/reset, RGB/overlay composition, project I/O, grayscale/RGB 8-bit and PNG 16-bit export, and Send to Edit work in large mode with normal-mode semantics.
- Cancellation and failures are transactional and do not corrupt channel artifacts, previews, project state, or output files.
...
1. [x] Core data, I/O, and services
   - Ownership: `internal/{fitsio,models,instrument,badpix,export,histogram,render,stretch,utils,config,debuglog,debugtime,version}` and their tests.
   - Review FITS parsing/writing, artifact lifetime, persistence compatibility, numeric/empty-input handling, and output error paths.
   ...
   - Validation: `go test ./internal/fitsio ./internal/models ./internal/instrument ./internal/badpix ./internal/export ./internal/histogram ./internal/render ./internal/stretch ./internal/utils ./internal/config ./internal/debuglog ./internal/debugtime ./internal/version`.

2. [x] Processing engine
   - Ownership: `internal/processing` and its tests.
   - Review WCS/alignment/catalog transforms, interpolation and dimensions, disk compositor boundedness and cancellation, cleaning/masks, NaN/short-slice behavior, and goroutine/resource safety.
   ...
   - Completed: fixed disk-compositor edge clipping, reference-grid affine ordering, and transactional preview publication. Added direct WCS-failure resize-fallback coverage. Independent review approved the changes; focused and full test/vet runs pass.
...
4. [x] Compose/Edit/Examine and shared UI
   - Ownership: Compose/Edit/Examine workspace and shared UI files, including `compose_*.go`, `workspace_compose.go`, `workspace_edit.go`, `workspace_examine.go`, `viewport.go`, and their matching tests; excludes mosaic/artifact/alignment UI files in lane 5.
   - Review job generation/cancellation/stale completion, Fyne-thread confinement, project load/save/reset parity, large-store cleanup, bounded previews and Edit handoff.
